    ...
    const optimizeCmd = vscode.commands.registerCommand('bytesized.optimizeCode', async () => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                await vscode.commands.executeCommand('vscode.diff', originalUri, optimizedDoc.uri, `ByteSized: ${document.fileName.split(/[\\/]/).pop()} (${languageId} Original ↔ Optimized)`);
                statusBarItem.text = `$(check) ByteSized: ${data.issues.length} Resolved`;
                vscode.window.showInformationMessage(`ByteSized: Resolved ${data.issues.length} issue(s)! ${data.summary || ''}`);
            }
            catch (err) {
                statusBarItem.text = "$(alert) ByteSized: Backend Offline";
                vscode.window.showErrorMessage(`ByteSized Error: Could not connect to backend at ${backendUrl}. Ensure uvicorn is running.`);
            }
        });
    });
