...
- Create `extensions/blueprintbob/` and `backend/routers/blueprintbob/` for the architecture visualizer extension.
...
│   │
│   └── blueprintbob/                    # Extension 2 — BlueprintBob visualizer
│       ├── package.json                 # depends on "@bytesized/shared": "file:../shared"
│       ├── tsconfig.json
│       ├── README.md
│       └── src/
│           └── extension.ts             # BlueprintBob activate/deactivate
│
├── backend/
...
│   ├── shared/
│   │   ├── __init__.py
│   │   └── models.py                    # RTLIssue, OptimizeRequest, BlueprintBobRequest, BlueprintBobResponse
│   ├── routers/
│   │   ├── __init__.py
│   │   ├── rtl/
│   │   │   ├── __init__.py
...
│   │   └── blueprintbob/                # BlueprintBob architecture engine
│   │       ├── __init__.py
│   │       ├── router.py                # APIRouter (/api/blueprintbob)
│   │       ├── generator.py
│   │       ├── compiler.py
│   │       └── analyzer.py
...
- `backend/routers/blueprintbob/router.py` holds an `APIRouter` with `GET /api/blueprintbob/health` and `POST /api/blueprintbob/generate`.
...
6. Create `backend/routers/blueprintbob/__init__.py`.
7. Create `backend/routers/blueprintbob/router.py` — `APIRouter(prefix="/api/blueprintbob", tags=["BlueprintBob"])` with `GET /health` and `POST /generate`.
8. Rewrite `backend/main.py` — app creation, CORS, then `app.include_router(rtl_router)` and `app.include_router(blueprintbob_router)`.
9. Smoke-test: `uvicorn main:app --reload` from `backend/` — confirm `/health`, `/api/optimize-rtl`, `/ws/test-room`, and `/api/blueprintbob/health` all respond.
...
### Sub-Task 4 — Scaffold the BlueprintBob Extension

**Status:** [x] done

**Intent:**
Scaffold the BlueprintBob architecture visualizer extension (`extensions/blueprintbob`)
and its backend service (`backend/routers/blueprintbob/router.py`).

**Expected Outcomes:**
- `extensions/blueprintbob/` exists with `package.json`, `tsconfig.json`, `README.md`, and `src/extension.ts`.
- `extensions/blueprintbob/package.json` depends on `@bytesized/shared` via `file:../shared`.
- The `extension.ts` activates and contributes `blueprintbob.visualizeWorkspace`.
- `backend/routers/blueprintbob/router.py` provides `/api/blueprintbob/health` and `/api/blueprintbob/generate` mounted in `main.py`.

**Todo List:**
1. Create `extensions/blueprintbob/package.json` with `blueprintbob.visualizeWorkspace`.
2. Create `extensions/blueprintbob/tsconfig.json`.
3. Create `extensions/blueprintbob/README.md`.
4. Create `extensions/blueprintbob/src/extension.ts`.
5. Run `npm install && npm run compile` inside `extensions/blueprintbob/` — confirm clean compile.

**Relevant Context:**
- `backend/routers/blueprintbob/` provides the architecture generation engine.
- Command name is `blueprintbob.visualizeWorkspace`.
...
| 2nd extension name | BlueprintBob (`blueprintbob`) |
